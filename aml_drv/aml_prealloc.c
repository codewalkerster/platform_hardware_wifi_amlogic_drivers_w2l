    ...
    switch (type) {
        ...
        case PREALLOC_TRACE_PTR_EXPEND:
            if (size > AML_PREALLOC_TRACE_PTR_EXPEND_BUF_SIZE) {
                AML_PRINT(AML_DBG_MODULES_CMD, "not enough pre-alloc buffer size(%ld) for trace ptr expend buf\n", size);
                return NULL;
            }
        ...
        case PREALLOC_TRACE_STR_EXPEND:
            if (size > AML_PREALLOC_TRACE_STR_EXPEND_BUF_SIZE) {
                AML_PRINT(AML_DBG_MODULES_CMD, "not enough pre-alloc buffer size(%ld) for trace str expend buf\n", size);
                return NULL;
            }
