...
dfs_pattern_detector_init(enum nl80211_dfs_regions region, u8 chain)
{
    ...
    dpd->region = NL80211_DFS_UNSET;
    dpd->enabled = AML_RADAR_DETECT_DISABLE;
    dpd->last_pulse_ts = 0;
    dpd->prev_jiffies = jiffies;
 ...
 * driver interface
 *****************************************************************************/
static u16 aml_radar_get_center_freq(struct aml_hw *aml_hw, u8 chain)
{
    if (chain == AML_RADAR_FCU)
        return aml_hw->phy.sec_chan.center1_freq;
...
static void aml_radar_detected(struct aml_hw *aml_hw)
{
#ifdef CONFIG_AML_SOFTMAC
    ieee80211_radar_detected(aml_hw->hw);
#else
    ...
    /*
      recopy chan_def in local variable because aml_radar_cancel_cac may
    ...
    */
    chan_def = aml_hw->chanctx_table[aml_hw->cur_chanctx].chan_def;

    aml_radar_cancel_cac(&aml_hw->radar);
    cfg80211_radar_event(aml_hw->wiphy, &chan_def, GFP_KERNEL);

#endif /* CONFIG_AML_SOFTMAC */
}

static void aml_radar_process_pulse(struct work_struct *ws)
{
    struct aml_radar *radar = container_of(ws, struct aml_radar,
                                            detection_work);
    struct aml_hw *aml_hw = container_of(radar, struct aml_hw, radar);
    enum aml_radar_chain chain;
    u32 pulses[AML_RADAR_LAST][AML_RADAR_PULSE_MAX] = {{0}};
    u16 pulses_count[AML_RADAR_LAST];
    ...
    for (chain = AML_RADAR_RIU; chain < AML_RADAR_LAST; chain++) {
        int start, count;

        count = radar->pulses[chain].count;
        start = radar->pulses[chain].index - count;
        if (start < 0)
            start += AML_RADAR_PULSE_MAX;

        pulses_count[chain] = count;
        if (count == 0)
            continue;

        if ((start + count) > AML_RADAR_PULSE_MAX) {
            u16 count1 = (AML_RADAR_PULSE_MAX - start);
    ...
    for (chain = AML_RADAR_RIU; chain < AML_RADAR_LAST; chain++) {
        ...
        freq = aml_radar_get_center_freq(aml_hw, chain);
            ...
            if (dfs_pattern_detector_add_pulse(radar->dpd[chain], chain,
                ...
                if (chain == AML_RADAR_RIU) {
                    ...
                    if (radar->dpd[chain]->enabled == AML_RADAR_DETECT_REPORT) {
                        ...
                        aml_radar_detection_enable(radar,
...
static void aml_radar_cac_work(struct work_struct *ws)
{
    struct delayed_work *dw = container_of(ws, struct delayed_work, work);
    struct aml_radar *radar = container_of(dw, struct aml_radar, cac_work);
    struct aml_hw *aml_hw = container_of(radar, struct aml_hw, radar);
    ...
    ctxt = &aml_hw->chanctx_table[radar->cac_vif->ch_index];
...
#endif
    aml_send_apm_stop_cac_req(aml_hw, radar->cac_vif);
    aml_chanctx_unlink(radar->cac_vif);
...
bool aml_radar_detection_init(struct aml_radar *radar)
{
    ...
    radar->dpd[AML_RADAR_RIU] = dfs_pattern_detector_init(NL80211_DFS_UNSET,
                                                           AML_RADAR_RIU);
    if (radar->dpd[AML_RADAR_RIU] == NULL)
        return false;

    radar->dpd[AML_RADAR_FCU] = dfs_pattern_detector_init(NL80211_DFS_UNSET,
                                                           AML_RADAR_FCU);
    if (radar->dpd[AML_RADAR_FCU] == NULL) {
        aml_radar_detection_deinit(radar);
        return false;
    }

    INIT_WORK(&radar->detection_work, aml_radar_process_pulse);
#ifdef CONFIG_AML_FULLMAC
    INIT_DELAYED_WORK(&radar->cac_work, aml_radar_cac_work);
    radar->cac_vif = NULL;
#endif /* CONFIG_AML_FULLMAC */
    return true;
}
...
{
    if (radar->dpd[AML_RADAR_RIU]) {
        dfs_pattern_detector_exit(radar->dpd[AML_RADAR_RIU]);
        radar->dpd[AML_RADAR_RIU] = NULL;
    }
    if (radar->dpd[AML_RADAR_FCU]) {
        dfs_pattern_detector_exit(radar->dpd[AML_RADAR_FCU]);
        radar->dpd[AML_RADAR_FCU] = NULL;
    }
}
                           ...
                           enum nl80211_dfs_regions region)
{
    ...
    return (dfs_pattern_detector_set_domain(radar->dpd[AML_RADAR_RIU],
                                            region, AML_RADAR_RIU) &&
            dfs_pattern_detector_set_domain(radar->dpd[AML_RADAR_FCU],
                                            region, AML_RADAR_FCU));
}
...
void aml_radar_detection_enable_on_cur_channel(struct aml_hw *aml_hw)
{
    ...
    ctxt = &aml_hw->chanctx_table[aml_hw->cur_chanctx];
 ...
 *****************************************************************************/
static
int aml_radar_dump_pri_detector(char *buf, size_t len,
                                 struct pri_detector *pde)
{
                                     ...
                                     struct aml_radar *radar, u8 chain)
{
    ...
    if (buf == NULL) {
        ...
        for (i = 0; i < dpd->num_radar_types; i++) {
            list_for_each_entry(pde, &dpd->detectors[i], head) {
                size_needed += aml_radar_dump_pri_detector(NULL, 0, pde);
            }
    ...
    for (i = 0; i < dpd->num_radar_types; i++) {
        ...
        len -= res;
        list_for_each_entry(pde, &dpd->detectors[i], head) {
            res = aml_radar_dump_pri_detector(&buf[write], len, pde);
            write += res;
            len -= res;
        }
    }
...
int aml_radar_dump_radar_detected(char *buf, size_t len,
                                   struct aml_radar *radar, u8 chain)
{
    struct aml_radar_detected *detect = &(radar->detected[chain]);
