...
bit3 reg_rx_bw_mask
bit4 g_frame_type_mask
bit5 reg_rx_addr2_mask
bit6 reg_rx_addr1_mask
*/
struct csi_set_req
{
    u8_l mask;
...
struct early_bcn
{
    //1: open, 0: close
    bool early_bcn_mode;
    //if config detect other IE, check ie_counter IE to match detected IE_ID, the min num is 9
    uint8_t ie_counter;
    //config detect IE_ID
    uint8_t   element_1;
    uint8_t   element_2;
...
#ifdef CONFIG_AML_NAN_SUPPORT
// Message API of the NAN task
enum nan_msg_tag
{
    /// Request to start the NAN
    NAN_ENABLE_REQ = LMAC_FIRST_MSG(TASK_NAN),
    NAN_ENABLE_CFM,
    NAN_PUBLISH_REQ,
    NAN_SUBSCRIBE_REQ,
    NAN_FOLLOW_UP_REQ,
